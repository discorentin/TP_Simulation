...
#        print("Taux d'utilisation du centre de réparation : " + str(self.taux_utilisation_centre_reparation) + "\n")
    ...
    def debut_simulation(self):
        temp = self.date_simu + self.rng.exponential(2)
        self.arrive_temps.append(temp)
        self.echeancier.append((self.ARRIVEE_BUS, temp))
    ...
    def acces_controle(self):
        self.qc -= 1
        self.bc = 1
        temp = self.date_simu + self.rng.uniform(0.25, 13 / 12)
        self.duree_controle.append(temp - self.date_simu)
        self.echeancier.append((self.DEPART_CONTROLE, temp))
